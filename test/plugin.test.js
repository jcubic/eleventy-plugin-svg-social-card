...
describe('option validation (at registration)', () => {
    ...
    it('requires a template in single-card mode', () => {
        expect(() => socialCardPlugin(cfg(), { data: () => ({}) }))
            .toThrow(/`template` option is required/);
    });

    it('requires data to be a function', () => {
        expect(() => socialCardPlugin(cfg(), { template, data: {} }))
            .toThrow(/`data` option must be a function/);
    });

    it('rejects a card variant named "emit"', () => {
        expect(() => socialCardPlugin(cfg(), {
            cards: { emit: { template, data: () => ({}) } },
        })).toThrow(/reserved/);
    });

    it('rejects a non-object card variant', () => {
        expect(() => socialCardPlugin(cfg(), { cards: { a: 'nope' } }))
...
describe('template validation (eleventy.before)', () => {
    it('throws on a template that renders invalid XML', async () => {
        ...
        await expect(t.fire('eleventy.before', { runMode: 'build' }))
            .rejects.toThrow(/invalid XML/);
    });

    it('warns about URL-encoded Liquid braces', async () => {
        const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
        const t = setup({ template: path.join(FIX, 'encoded.svg'), data: () => ({ avatar: '/a' }) });
        await t.fire('eleventy.before', { runMode: 'build' });
        expect(warn).toHaveBeenCalledWith(expect.stringMatching(/URL-encoded Liquid placeholders/));
    });
});
...
describe('rendering', () => {
    it('writes a PNG and returns nothing without "emit"', async () => {
        ...
        await expect(fs.access(path.join(outputDir, 'hello.png'))).resolves.toBeUndefined();
    });
    ...
    it('passes raw values through when escape is false', async () => {
        const t = setup({ escape: false, data: () => ({ title: 'A & B', author: '', date: '' }) });
        await t.fire('eleventy.before', { runMode: 'build' });
        await t.call(page);
...
describe('multiple cards', () => {
    function multi(options) {
        ...
        socialCardPlugin(t.cfg, {
            cards: {
                article: {
                    template, outputDir, urlPath: '/a',
                    data: () => ({ title: 'A', author: '', date: '' }), delay: 0,
                },
                pages: {
                    template, outputDir, urlPath: '/p',
                    data: () => ({ title: 'P', author: '', date: '' }), delay: 0,
                },
                ...options,
            },
            browser: factory,
    ...
    it('resolves a nameless call when there is exactly one card', async () => {
        ...
        socialCardPlugin(t.cfg, {
            cards: { only: { template, outputDir, urlPath: '/o', data: () => ({ title: 'x', author: '', date: '' }), delay: 0 } },
            browser: factory,
        });
        await t.fire('eleventy.before', { runMode: 'build' });
